...
function F = stk_angular_velocity
    ...
    F.GetAngularVelocity = @GetAngularVelocity;
      ...
      F.GetAzimuth2 = @GetAzimuth2;
    F.GetPitch2 = @GetPitch2;
        F.delete_vector = @delete_vector;
...
% 获取方位角（Azimuth）速度
function  GetAzimuth2(root, satellite1_name, satellite2_name, starttime, endtime, timestep)
    % 输入验证
    if nargin < 3
        error('需要提供 root 对象和两个卫星名称。');
    end
...
% 获取俯仰角（Pitch）速度
function GetPitch2(root, satellite1_name, satellite2_name, starttime, endtime, timestep)
    % 输入验证
    if nargin < 3
        error('需要提供 root 对象和两个卫星名称。');
    end
...
% 获取俯仰角（Pitch）速度
function delete_vector(root, satellite1_name, satellite2_name, starttime, endtime, timestep)
    % 输入验证
    if nargin < 3
        error('需要提供 root 对象和两个卫星名称。');
    end
